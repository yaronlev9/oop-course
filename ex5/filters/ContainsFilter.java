package filters;
import java.io.File;

/**
 * The API ContainsFilter.
 *  a class for the ContainsFilter that implements Filter.
 * @author oop
 */
public class ContainsFilter implements Filter {

    /**the string that is contained in file name*/
    private String value;

    /**a boolean instance of activating reverse filter or not */
    private boolean noActivated;

    /**
     * Constructs a new ContainsFilter object.
     *
     * @param value the string that is contained in file name
     * @param noActivated the boolean instance for activating reverse filter
     */
    public ContainsFilter (String value ,boolean noActivated){
        this.value = value;
        this.noActivated = noActivated;
    }

    /**
     * Checks if the file given passes the filter.
     * @param file the file
     * @return true if the file passes the filter false if not.
     */
    @Override
    public boolean isPass(File file){
        if (!noActivated) {
            return file.getName().contains(value);
        } else {
            return !file.getName().contains(value);
        }
    }
}
